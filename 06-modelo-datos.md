...
| ------------------- | --------------------- | --------------------------------------------- |
| `TokenFixture`      | raíz de una ejecución | ID estable, versión y timestamps normalizados |
| `Header`            | dato de dominio 1     | ID estable, versión y timestamps normalizados |
| `Claims`            | dato de dominio 2     | ID estable, versión y timestamps normalizados |
| `ValidationPolicy`  | dato de dominio 3     | ID estable, versión y timestamps normalizados |
| `ValidationFinding` | dato de dominio 4     | ID estable, versión y timestamps normalizados |

```mermaid
erDiagram
    TokenFixture ||--o{ Header : relates
    Header ||--o{ Claims : relates
    Claims ||--o{ ValidationPolicy : relates
    ValidationPolicy ||--o{ ValidationFinding : relates
```
